...
class Form_Styler extends Widget_Base {
	...
	protected function register_style_controls(): void {
		...
		$this->add_group_control(
			...
			array(
				...
				'selector'       => '{{WRAPPER}} .mc4wp-form-fields input[type="submit"],{{WRAPPER}} .wpcf7-submit, {{WRAPPER}} .submit-wrap .ninja-forms-field, {{WRAPPER}} .wpforms-submit, {{WRAPPER}} .wpforms-page-next, {{WRAPPER}} .wpforms-page-previous,{{WRAPPER}} #contact-btn',
			)
		);
	...
	protected function render_cf7_template( $settings ) {
		if ( ! empty( $settings['cf7_templates'] ) && 'empty' !== $settings['cf7_templates'] ) {
	...
	protected function render_wpforms_template( $settings ) {
		if ( ! empty( $settings['wpforms_templates'] ) && 'empty' !== $settings['wpforms_templates'] ) {
			if ( function_exists( 'wpforms_display' ) ) {
				echo wp_kses_post(
					wpforms_display(
						esc_attr( $settings['wpforms_templates'] ),
						isset( $settings['show_form_title'] ) && 'yes' === $settings['show_form_title'],
						isset( $settings['show_form_description'] ) && 'yes' === $settings['show_form_description']
					)
				);
			}
		}
	...
	protected function render_ninja_forms_template( $settings ) {
		if ( ! empty( $settings['ninja_forms_templates'] ) && 'empty' !== $settings['ninja_forms_templates'] ) {
	...
	protected function render_mailchimp_forms_template( $settings ) {
		if ( ! empty( $settings['mailchimp_form_templates'] ) && 'empty' !== $settings['mailchimp_form_templates'] ) {
