...
/**
 * REST Endpoint for Elementify Addons plugin.
 *
...
class Rest_Endpoint
{
    ...
    public function get_item_permissions_check(WP_REST_Request $request)
    {
        if (!current_user_can('manage_options')) {
            return new WP_Error(
                'rest_forbidden',
                __('You do not have permission to access this resource.', 'elementify-addons-for-elementor'),
    ...
    public function update_item(WP_REST_Request $request)
    {
        ...
        if (is_wp_error($validation)) {
            return new WP_Error(
                'rest_invalid_params',
                __('Invalid settings data provided.', 'elementify-addons-for-elementor'),
    ...
    public function get_item_schema(): array
    {
         ...
         */
        $schema = apply_filters('elementify_addons_for_elementor_options_rest_schema', $schema);
    ...
    protected function prepare_value($value, array $schema)
    {
        ...
        if (is_null($sanitized_value)) {
            return new WP_Error(
                ...
                __('The settings data could not be sanitized.', 'elementify-addons-for-elementor'),
                ['status' => 400]
            );
        }
