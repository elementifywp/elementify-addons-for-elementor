...
const TemplateImporter = ({ isOpen, onClose }) => {
    ...
    async (forceUpdate = false) => {
      ...
      try {
        ...
        if (!response?.success || !Array.isArray(response.data)) {
          throw new Error(
            __("Failed to load templates", "elementify-addons-for-elementor"),
          );
        }
        ...
        setCategories([
          {
            slug: "all",
            name: __("All Templates", "elementify-addons-for-elementor"),
          },
          ...typeCategories,
        ]);
      } catch (err) {
  ...
  const importTemplate = useCallback(
    async (template) => {
      if (!template?.id) {
        setError(
          __("Invalid template selected.", "elementify-addons-for-elementor"),
        );
        return;
      }
      ...
      try {
        ...
        if (data.data?.elements && window.elementor) {
          ...
          elementor.notifications.showToast({
            message:
              data.data?.message ||
              __("Template imported.", "elementify-addons-for-elementor"),
            type: "success",
          });
        }
  ...
  return (
    <>
      <Modal
        title={__("Import Templates", "elementify-addons-for-elementor")}
            ...
            <FlexBlock>
              {previewTemplate ? (
                <div className="eae-preview-actions">
                  ...
                  >
                    {__("Back to Library", "elementify-addons-for-elementor")}
                  </Button>
                </div>
              ) : (
                // Your else condition content here
                <div className="eae-logo-wrap">
                  <img src={logo} alt="Elementify" className="eae-modal-logo" />
                  <h2 className="eae-modal-title">
                    {__("Library", "elementify-addons-for-elementor")}
                  </h2>
                </div>
              )}
            </FlexBlock>
              ...
              {!previewTemplate && (
                <ButtonGroup>
                  <Button
                    ...
                    onClick={() => handleTypeChange("section")}
                    icon={<FiBox />}
                  >
                    {__("Blocks", "elementify-addons-for-elementor")}
                  </Button>
                  <Button
                    ...
                    icon={<FiLayout />}
                  >
                    {__("Pages", "elementify-addons-for-elementor")}
                  </Button>
                </ButtonGroup>
              )}
            </FlexBlock>
            ...
            <FlexItem>
              <div className="eae-modal-actions">
                {previewTemplate ? (
                  ...
                  <Button
                    icon={<FiRefreshCcw />}
                    label={__("Refresh", "elementify-addons-for-elementor")}
                ...
                <Button
                  icon={<FiX />}
                  label={__("Close", "elementify-addons-for-elementor")}
                  onClick={handleClose}
                  variant="secondary"
                />
              </div>
        ...
        <div className="eae-modal-body">
          {previewTemplate ? (
            <div className="eae-preview-container responsive-device-wrapper">
              ...
              >
                <ResponsiveIframeViewer
                  src={previewTemplate.url || "https://demo.elementifywp.com/"}
                  title={previewTemplate.title || "Template Preview"}
                  onIframeLoad={() => setPreviewLoading(false)}
                  // Start in desktop mode with your custom size
                  size={viewport}
                  // Your custom dimensions for each toggle button
                  overrideViewportSizes={{
                    [ViewportSize.desktop]: { width: "100%" }, // initial + when clicking desktop button
                    [ViewportSize.tablet]: { width: "768px" },
                    [ViewportSize.mobile]: { width: "375px" },
                  }}
                ...
                <Flex align="center" justify="space-between" gap={4}>
                  <FlexBlock className="eae-search-wrapper">
                    ...
                    <span className="eae-template-count">
                      {loading
                        ? __("Loading...", "elementify-addons-for-elementor")
                      ...
                      <HoverDropdown>
                        <div className="eae-category-tabs">
                          <TabPanel
                            ...
                            initialTabName={selectedCategory}
                  ...
                  <div className="eae-no-results">
                    ...
                    {(searchTerm || selectedCategory !== "all") && (
                      <Button variant="secondary" onClick={handleClearFilters}>
                        {__("Clear filters", "elementify-addons-for-elementor")}
                      </Button>
                    )}
                  </div>
                ) : (
                    ...
                    <div className="eae-template-grid">
                      {displayedTemplates.map((template) => (
                        <TemplateCard
                          ...
                          isImporting={!!importingId} // Change this line
                        />
                      ))}
                    </div>
